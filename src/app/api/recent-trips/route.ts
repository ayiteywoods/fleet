...
import { withAuthAndDatabaseFallback, recentTripsHandlers } from '@/lib/apiWrapper'
...
// GET - Fetch recent trips based on user permissions
export const GET = withAuthAndDatabaseFallback(async (request: NextRequest, user: any) => {
  try {
    // Try to use the mock handler first
    return await recentTripsHandlers.getRecentTrips(request, user)
  } catch (error: any) {
    console.error('Error in recent trips handler:', error)
