...
export default function RolesModal({ isOpen, onClose }: RolesModalProps) {
  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      const tableData = filteredAndSortedRoles.map(role => `
        <tr>
          <td>${role.name}</td>
          <td>${role.guard_name}</td>
          <td>${role.created_at ? new Date(role.created_at).toLocaleDateString() : 'N/A'}</td>
          <td>${role.updated_at ? new Date(role.updated_at).toLocaleDateString() : 'N/A'}</td>
        </tr>
      `).join('')
            ...
            <div className="overflow-x-auto">
              {loading ? (
                ...
                <div className="text-center py-8 text-gray-500">
                  {searchQuery ? 'No roles match your search.' : 'No roles found. Add some to get started.'}
                </div>
              ) : (
                <>
                      ...
                      <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        <div>
                          <p className={`text-sm ${
                            ...
                            <span className="font-medium">{Math.min(endIndex, filteredAndSortedRoles.length)}</span> of{' '}
                            <span className="font-medium">{filteredAndSortedRoles.length}</span> results
                          </p>
                        </div>
                        <div>
