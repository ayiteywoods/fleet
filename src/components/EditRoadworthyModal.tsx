...
interface EditRoadworthyModalProps {
  isOpen: boolean
  onClose: () => void
...
export default function EditRoadworthyModal({ isOpen, onClose, onSubmit, roadworthyRecord }: EditRoadworthyModalProps) {
    ...
    >
      <div className={`relative p-6 rounded-xl shadow-lg w-full max-w-2xl mx-4 my-8 ${themeMode === 'dark' ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}`}>
        <div className="flex items-center justify-between pb-4 border-b border-gray-200 dark:border-gray-600">
          <h2 className="text-2xl font-bold">Edit Roadworthy Record</h2>
        ...
        {prefilling ? (
          ...
          <div className="flex justify-end space-x-4 mt-6">
            ...
            >
              {loading ? 'Updating...' : 'Update Roadworthy'}
            </button>
          </div>
        </form>
        )}
