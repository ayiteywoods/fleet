...
export default function ViewPermissionModal({ isOpen, onClose, permission }: ViewPermissionModalProps) {
  ...
  const handleViewExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Permission Details')
    XLSX.writeFile(wb, `permission-${permission.name.replace(/\s+/g, '-').toLowerCase()}.xlsx`)
  }
  ...
  const handleViewExportCSV = () => {
    ...
    a.href = url
    a.download = `permission-${permission.name.replace(/\s+/g, '-').toLowerCase()}.csv`
    a.click()
  ...
  const handleViewExportPDF = () => {
    ...
    // Add title
    doc.setFontSize(20)
    doc.text('Permission Details Report', 14, 22)
    
    // Add permission name
    doc.setFontSize(16)
    doc.text(`Permission: ${permission.name}`, 14, 35)
    ...
    doc.setFontSize(10)
    doc.text(`Permission ID: ${permission.id}`, 20, yPosition)
    yPosition += 8
    doc.text(`Name: ${permission.name}`, 20, yPosition)
    yPosition += 8
    doc.text(`Guard Name: ${permission.guard_name}`, 20, yPosition)
    yPosition += 15
    ...
    doc.setFontSize(10)
    doc.text(`Created At: ${permission.created_at ? new Date(permission.created_at).toLocaleDateString() : 'N/A'}`, 20, yPosition)
    yPosition += 8
    doc.text(`Updated At: ${permission.updated_at ? new Date(permission.updated_at).toLocaleDateString() : 'N/A'}`, 20, yPosition)
    
    doc.save(`permission-${permission.name.replace(/\s+/g, '-').toLowerCase()}.pdf`)
  }
  ...
  const handleViewPrint = () => {
    ...
    if (printWindow) {
      printWindow.document.write(`
        <html>
          <head>
            <title>Permission Details Report</title>
            <style>
          ...
          <body>
            <div class="header">
              <h1>Permission Details Report</h1>
            ...
            <div class="section">
              ...
              <div class="field">
                ...
                <span class="value">${permission.name}</span>
              </div>
              <div class="field">
                <span class="label">Guard Name:</span>
                <span class="value">${permission.guard_name}</span>
              </div>
            </div>
            ...
            <div class="section">
              ...
              <div class="field">
                ...
                <span class="value">${permission.created_at ? new Date(permission.created_at).toLocaleDateString() : 'N/A'}</span>
              </div>
              <div class="field">
                <span class="label">Updated At:</span>
                <span class="value">${permission.updated_at ? new Date(permission.updated_at).toLocaleDateString() : 'N/A'}</span>
              </div>
            </div>
